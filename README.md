...
2. **站点配置**：编辑 `index.html` 顶部的「站点配置」区块：
   ...
   - `SUBMIT_URL`：申请提交接口，默认 `https://api.frdlink.link/links/submit.php`（POST JSON，返回 `{ok, message}`）；留空 `""` 则回退 mailto；也可改为 Formspree 等第三方地址
   - 两个接口均已部署在 `api.frdlink.link`（检测 = `links/link_checker.php`，提交 = `links/submit.php`，CORS 已开启，静态页可直接跨域调用）
3. **本地预览**：直接用浏览器打开 `index.html`，或运行 `npx serve .` 启动本地服务（本机打开时申请自动走 mailto）。
4. **部署方式一：纯静态（默认）**：`index.html` + `links.js` + `favicon.ico` 上传到任意静态托管（GitHub Pages、Vercel 等）即可，`SUBMIT_URL` 留空，申请走 mailto。
5. **部署方式二：静态页面 + 远程 PHP（推荐）**：页面主体仍放纯静态托管，把 `submit.php` 单独上传到支持 PHP 的空间（如你的虚拟主机），并在 `index.html` 的 `SUBMIT_URL` 填 PHP 完整地址，如 `"https://你的php域名/submit.php"`。`submit.php` 已开启跨域（CORS），静态页面直接调用；申请自动写入 PHP 空间的 `applications.json`。建议把 `links.js` 也同步上传到 PHP 空间同目录，保证服务端查重准确。
6. **部署方式三：同目录 PHP**：`index.html` 与 `submit.php` 同目录部署在 PHP 空间，`SUBMIT_URL` 填 `"submit.php"`，申请自动写入同目录 `applications.json`。
   - 方式二/三共用待审列表 `applications.json`：定期查看、确认后把条目加入 `links.js` 即可；可选编辑 `submit.php` 顶部配置开启 `mail()` 邮件通知。
